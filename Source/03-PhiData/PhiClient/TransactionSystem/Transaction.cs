...
namespace PhiClient.TransactionSystem
{
	...
	public abstract class Transaction : IDable
	{
		...
		{
			return this.state == TransactionResponse.ACCEPTED || this.state == TransactionResponse.DECLINED || this.state == TransactionResponse.INTERRUPTED || this.state == TransactionResponse.TOOFAST;
		}
		...
		{
			RealmData realmData = ((RealmContext)c.Context).realmData;
			if (realmData != null)
			{
				this.sender = ID.Find<User>(realmData.users, this.senderId);
				this.receiver = ID.Find<User>(realmData.users, this.receiverId);
			}
		}
