...
namespace PhiClient.UI
{
	...
	public class TabsContainer : Displayable
	{
		...
		public void AddTab(string label, Displayable displayable)
		{
			...
			TabRecord tab = new TabRecord(label, delegate()
			{
				this.selectedTab = index;
			}, this.selectedTab == index);
			this.tabs.Add(new TabEntry
			{
				tab = tab,
		...
		{
			TabDrawer.DrawTabs(GenUI.TopPartPixels(inRect, 45f), (from o in this.tabs
			select o.tab).ToList<TabRecord>(), 200f);
			Rect inRect2 = GenUI.BottomPartPixels(inRect, inRect.height - 45f);
			this.tabs[this.selectedTab].displayable.Draw(inRect2);
		}
